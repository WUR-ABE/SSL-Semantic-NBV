...
int main(int argc, char** argv)
{
  ...
  std::string arm_name, usecase = argv[1];
  ...
  if (obstacles.getType() == XmlRpc::XmlRpcValue::TypeArray)
  {
    for (int i = 0; i < obstacles.size(); i++)
    {
      ...
      dim = utils::vector3(static_cast<double>(obstacle[0]), static_cast<double>(obstacle[1]),
                           static_cast<double>(obstacle[2]));
      dims.push_back(dim);
      geometry_msgs::Pose obstacle_pose;
      obstacle_pose =
          utils::pose(utils::point(static_cast<double>(obstacle[3]), static_cast<double>(obstacle[4]),
                                   static_cast<double>(obstacle[5])),
                      utils::quaternion(static_cast<double>(obstacle[6]), static_cast<double>(obstacle[7]),
                                        static_cast<double>(obstacle[8]), static_cast<double>(obstacle[9])));
      obstacle_poses.poses.push_back(obstacle_pose);
    }
  }

  // Object instantiations.
  arm_control::ArmControl arm_control(arm_name, usecase);

  arm_control.AddCollisionObjects(dims, obstacle_poses);

  ros::ServiceServer move_arm_to_pose_srv =
      nh.advertiseService("move_arm_to_pose", &arm_control::ArmControl::MoveToGoalSrv, &arm_control);
