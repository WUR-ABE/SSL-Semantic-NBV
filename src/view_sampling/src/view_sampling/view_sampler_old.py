...
class ViewSampler:
    def __init__(self, distribution_type:str):
        ...
        self.embedder, self.out_dim = get_embedder(multires=10, i=0)
        self.interv_h = 0.05
        self.interv_ang = 10
    ...
    def sample_views(self, **kwargs) -> np.array:
        args = {
            ...
            "plane_views": {
                "distance": {"x": -0.6, "y": 0, "z": 0},
                "interval": {"dh": 0.1, "dw": 0.1},
                "plane_size": {"height": 0.6, "width": 1},
            },
            "cylinder_views": {
    ...
    def plane_distribtion(self) -> np.array:
        self.plane_center = np.array([self.center[0] + self.kwargs['plane_views']['distance']['x'], 
                                      self.center[1]+self.kwargs['plane_views']['distance']['y'], 
                                      self.center[2]+self.kwargs['plane_views']['distance']['z']])
    ...
    def visual_plane(self):
        marker = Marker()
        marker.header.frame_id = self.ref_frame
        marker.id = 0
        marker.type = Marker.CUBE
        marker.action = marker.ADD
        marker.scale = Vector3(0.01,self.plane_width,self.plane_height)
...
if __name__ == '__main__':
    ...
    config = {'num_col': 12, 'num_row':18}

    distribution_type = 'cylinder'  #cylinder, cylinder
    viewsampler = ViewSampler(distribution_type)
    viewpoints = viewsampler.sample_views(**config)
    ...
    while True:
        ...
        while in_bound:
            ...
            rospy.sleep(50)
            action = 3
            view, hw, in_bound = ViewSampler.update_view(viewpoints, hw, action=0)
