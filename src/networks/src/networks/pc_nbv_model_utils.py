...
def normalize_point_batch(pc, NCHW=True):
    """
    Normalize a batch of point clouds with an additional class channel.
    :param
        pc      [B, N, 4] or [B, 4, N] (x, y, z, class)
        NCHW    if True, treat the second dimension as the channel dimension
    :return
        pc      normalized point clouds, same shape as input
        centroid [B, 1, 3] or [B, 3, 1] center of (x, y, z)
        furthest_distance [B, 1, 1] scale of (x, y, z)
    """
    spatial_dims = slice(0, 3)  # Indices of (x, y, z)
    point_axis = 2 if NCHW else 1
    dim_axis = 1 if NCHW else 2

    # Calculate centroid and scale based only on (x, y, z)
    centroid = torch.mean(pc[..., spatial_dims], dim=point_axis, keepdim=True)
    pc[..., spatial_dims] = pc[..., spatial_dims] - centroid

    furthest_distance, _ = torch.max(
        torch.sqrt(torch.sum(pc[..., spatial_dims] ** 2, dim=dim_axis, keepdim=True)),
        dim=point_axis,
        keepdim=True,
    )
    pc[..., spatial_dims] /= furthest_distance
...
class DenseEdgeConv(nn.Module):
    ...
    def get_local_graph(self, x, k, idx=None):
        ...
        if idx is None:
            # BCN(K+1), BN(K+1)
            knn_point, idx, _ = group_knn(k + 1, x, x, unique=False)
            idx = idx[:, :, 1:]
...
class Feature_Extraction(torch.nn.Module):
    ...
    def __init__(self, dense_n=3, growth_rate=12, knn=16, in_channels=7):
    ...
    def forward(self, xyz_normalized, previous_level4=None, **kwargs):
        ...
        x = self.layer0(xyz_normalized.unsqueeze(dim=-1)).squeeze(dim=-1)# add dropout after layer0 and set it always actived
        x = F.dropout(x, p=0.5) if self.dropout else x

        if "phase" in kwargs and kwargs["phase"] == "vis":
            self.vis["layer_0"] = x

        y, idx = self.layer1(x)
        y = F.dropout(y, p=0.5) if self.dropout else y
...
class SelfAttention(nn.Module):
    def __init__(self, in_channel, gain=1, dropout=False):
        super().__init__()
    ...
    def forward(self, input):
        ...
        query = F.dropout(query, p=0.5) if self.dropout else query
        key = F.dropout(key, p=0.5) if self.dropout else key
        value = F.dropout(value, p=0.5) if self.dropout else value
