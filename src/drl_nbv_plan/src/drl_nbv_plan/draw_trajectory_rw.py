...
def transform_pcd(pcd, x, y, z, rotation, scale=1, downsample_voxel_size=0.003):
    ...
    pcd = point_cloud_down_sampling(pcd, downsample_voxel_size)
    return pcd
...
def vis_trajectory_with_frames(pcd, viewpoints, all_viewpoints, save_path=None):
    ...
    colors = pcd[:, 3:6]
    # set to GREY
    # colors[roi_index==0, :] = [0.5, 0.5, 0.5]  # Grey for ROI points
    colors[roi_index==0, :] = [0.7, 0.7, 0.7]  # light Grey for ROI points
    ...
    opt.point_size = 2  # adjust as needed
    ...
    center =  np.array([0.3, -1, 0.9])
    ctr = vis.get_view_control()
    ctr.set_lookat(center)
    ctr.set_up([0, 0, 1])
    # Adjust front vector to look forward along the -y -axis
    # Using a combination of y and Z components to achieve the downward angle
    ctr.set_front([0, np.cos(np.radians(-5)), -np.sin(np.radians(2))])
...
if __name__ == '__main__':
    ...
    object_name = 'NODE'  # Change this to 'PLANT' or 'NODE' as needed
    planner = 'ssl_semantic_nbv'

    complete_pcd_dir = '/home/jianchao/dataset/nbv_data/paper4/gt_pcd'
    results = pd.read_csv(f'/home/jianchao/ros_workspace/paper4_semantic_nbv/src/drl_nbv_plan/materials/prilinary_test/real_world_experiments/{object_name}/results.csv')
    
    output_dir = f'/home/jianchao/ros_workspace/paper4_semantic_nbv/src/drl_nbv_plan/materials/prilinary_test/real_world_shortcut/short_cut/{object_name}'
    ...
    for trajectory_index in range(20):
        ...
        viewpoints_dir = '/home/jianchao/dataset/nbv_data/paper4/data_smaller_fov_distance'
        viewpoints = np.load(os.path.join(viewpoints_dir, 'viewpoints_%s.npy'%str(plant)), allow_pickle=True)
        ...
        fruit_poses, fruit_sizes, node_poses, node_sizes = get_rios(int(plant), rotation, [x, y, z])
