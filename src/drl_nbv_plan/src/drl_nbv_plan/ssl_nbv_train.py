...
class Trainer:
    def __init__(self):
        ...
        'experiment': 1, # similated normal plants, 1: simulated heavy occlusion plants, 2: real plants
    ...
    def create_agent(self):    
        if self.planner.startswith('ssl_semantic_nbv'):
            self.agent = AgentGlobal(self.environment.config_middle['num_col']*self.environment.config_middle['num_row'], self.planner)
            ...
            self.agent = AgentGlobalWork1(self.environment.config_middle['num_col']*self.environment.config_middle['num_row'])
            self.agent.config_train(
                buffer_size=self.buffer_size,
                batch_size=self.batch_size,
                update_number=self.update_number,)
    ...
    def run(self):
        ...
        for planner in self.config['planners']:
            ...
            for i in tqdm(range(self.max_episode+1)):
                ...
                if valid_loss<self.episodic_loss and self.episode_count>1000:
